...
- [x] Write RED tests using the exact boundary cases and public APIs:
...
- [x] Run `SRT_REAL_EXPORT=1 SRT_FFMPEG_PATH=/opt/homebrew/opt/ffmpeg-full/bin/ffmpeg SRT_FFPROBE_PATH=/opt/homebrew/opt/ffmpeg-full/bin/ffprobe node --test tests/group-video-export.test.js` and record RED.
- [x] Implement source-split transparent branch using source timestamps, not an independent color-source clock. Proven local structure:
...
- [x] Write RED desktop case submitting a fixture group Recipe, then use normal seek/play/pause/ended events to assert alpha and bounds at beginning/mid/overshoot/end; no manual render calls to patch playback. Include reverse seek, portrait/clipping, group-before-subtitle order, two overlapping groups, reload, next-request nested context, second add-only request and whole-request undo. Add invalid second group check for zero partial commit. Fixture model responses are only deterministic regression evidence.
- [x] Run new spec to establish RED before app/default registration changes.
- [x] Extend drawGraph's visual node dispatch and source-ready check to include the group; preserve static branches and frame lifecycle. Reuse one scratch surface per target, resize/reset as needed, draw children in local edit order then transform once. For incoming sampled params, avoid applying keyframes twice; actual group range/time should be consumed once by sample.
- [x] Keep timeline aggregation by request. Static-only label remains `静态图层`; when a request contains a group, label `动画图层`. Use generic timeline metadata to total child elements instead of counting group edits as one element; group metadata optional for old static records, so old tests/counts remain unchanged. No new panel or keyframe UI.
- [x] Serialize full parameter schemas into prompt. Teach relative-to-range-start time, fixed pivot and newly-added group semantics in descriptions, not effect→recipe examples. Preserve incremental steps contract. Valid nested group payloads enter next context; undeclared fields are omitted/rejected using shared matching. An instruction to modify an existing edit in place must not falsely claim replacement; current add-only limitations remain explicit.
- [x] Run focused Node/UI then real-enabled full Node and Electron suites once. Commit `feat: preview AI-composed group animations in editor`; write `.superpowers/sdd/cycle5-task-3-report.md`, including visible checkpoint/artifacts and exact counts.
...
- [x] Parent independently runs final real-enabled Node suite and full Electron suite on final code; final whole-cycle read-only review uses baseline b530e2d.
- [x] Production Electron isolated project on synthetic footage: run actual natural-language animation request through Claude, project persistence, natural playback/seeking, UI export/real FFmpeg. Then another add-only request with current nested context, undo and reload. Only send artificial instructions/capability/project metadata, never user footage or paths; follow tool authorization and ask if external calls are rejected. Do not call a mock test real-AI acceptance.
- [x] At identical start/middle/overshoot/settled/end media times compare preview/export direction, pivot/size and interior alpha against source pixels. Solid-patch codec tolerance 7/255 (do not silently raise), shared integer geometry +/-1px away from antialiased boundaries. Inspect text in actual frames; no promise of identical glyph rasterization. Use ordinary playback and backwards seek to prove deterministic timing, no wall-clock drift.
- [x] Log internal completion separately from user-visible result, exact test counts and actual AI/MP4 evidence, own-footage/manual-save-dialog gaps. Record six-hour ceiling/exit. Local commits only; leave tested worktree for manual user viewing, do not start Cycle 6.
...
## Current outcome — 2026-09-08

Development and local acceptance complete at product commit443f523; actual-Claude gate subsequently passed after explicit user consent for exactly two calls. Start07:56UTC, desktop visible checkpoint08:58UTC, final local production smoke09:15UTC, within six-hour ceiling. The authorized follow-up changes only acceptance records, not product code. No Cycle6 or automatic new push.

- Task1 reviewed after minimal sparse-array fixcfdcb5c; Task2 and Task3 reviews Approved. Final whole-cycle b530e2d..443f523 review Approved, no outstanding findings.
- Parent fresh final real-enabled Node/media314/314 and Electron57/57, exit0, no failures/skips. Desktop artifacts /tmp/srt-cycle5-final.6nyXph/e2e/.
- Production-local isolated Electron/IPC/parser/FFmpeg with fixed translations PASS: fade, scale, overshoot, hold, half-open end; natural play/seek/pause/ended; reverse repeatability; exact project reload; second new text request receives nested group context and undo/reload preserves original group. Real640x3604sH264/AAC MP4. Six animated frames/1013 solid samples max7/255; geometry within1px. Source frame marker independently confirms frame identity; no glyph-byte parity claimed.
- Original local attempt /tmp/srt-cycle5-local.DwRXjn failed at nonrepresentable31/24 seek time. Product-free /tmp/srt-cycle5-seek-probe.qKpJ2i proved Chromium truncates1.291666666 to1.291666 and selects previous frame, even after3animationframes. Only temporary harness seek alignment corrected; original failure retained, no product change or tolerance increase. Final local evidence /tmp/srt-cycle5-local-final.hri4Fa/.
- At the initial local handoff no real Claude calls had been made and explicit consent was pending. User then replied “允许。”; original production harness /tmp/srt-cycle5-real.qMHHAs/run.cjs ran once with default Claude/export services and only a fixed temporary save destination. Exactly two translations (50.186s/10.288s) and one export, PASS/exit0, no page errors; no footage, paths or real project data sent to translation. First actual response is one group with shape/two-line text, opacity keyframes and back-out scale; next context carries its full canonical parameters and revision1, next result adds only one text edit, undo/reload retains the old group.
- Actual-AI media evidence: real640x3604sH264/AAC;13previewtimes/10exportframes,6activeframes/765solidpatches max6/255, geometry within1px, same source-frame alignment and unchanged tolerances. Parent viewed editor preview and export-at-1.5s.png. Evidence /tmp/srt-cycle5-real.qMHHAs/{evidence.json,ipc-evidence.json,export.mp4}. Earlier full314/314 and57/57 were not rerun for this documentation-only follow-up. Own-footage/native save-dialog manual experience remains separate; second request was not re-exported, and whole-group first-request undo has prior deterministic coverage, not this actual-AI run.
- Independent read-only real-evidence audit Approved, no blocking issues; verified production defaults, exact call count, synthetic-data-only translation payloads, media and state assertions. This is development Electron acceptance, not packaged-distribution acceptance.
